...
from synthetic_data_MLP.synthetic_data_generation import *
...
def estimate_e(X, A):
    ...
    model = tf.keras.models.Sequential([
        tf.keras.layers.Dense(64, activation='relu', input_dim=X.shape[1]),  # First hidden layer
    ...
    # Train the model
    model.fit(X_train, A_train, validation_data=(X_test, A_test), epochs=50, batch_size=32)

    # e = model_e.fit(X, A).predict_proba(X)[:, 1]
    e = model.predict(X)
    return e.reshape(-1, 1)

def estimate_mu(X, A, y):
    '''
        Estimate response function using a model_y
    '''
    train_data = np.concatenate((X, A), axis=1)

    train_data_train, train_data_test, y_train, y_test = train_test_split(train_data, y, test_size=0.2, random_state=42)

    # model = Sequential([
    #     Dense(64, activation='relu', input_dim=train_data.shape[1]),  # First hidden layer
    #     Dense(32, activation='relu'),                             # Second hidden layer
    #     Dense(1, activation='linear')                             # Output layer for regression
    # ])

    # model.compile(optimizer=adam_v2.Adam(learning_rate=0.001), loss='mse', metrics=['mae'])
    model = tf.keras.models.Sequential([
        tf.keras.layers.Dense(64, activation='relu', input_dim=train_data.shape[1]),  # First hidden layer
        tf.keras.layers.Dense(32, activation='relu'),                             # Second hidden layer
        tf.keras.layers.Dense(1, activation='linear')                             # Output layer for regression
    ])
    ...
    # Train the model
    history = model.fit(train_data_train, y_train, validation_data = (train_data_test, y_test), epochs=50, batch_size=32)
    ...
    mu0 = model.predict(test_0)
    mu1 = model.predict(test_1)
