...
from mimic_data.mimic_data_generation import *
...
def data_generation(gp_params, all_covs, big_n_rct, big_n_obs, n_MC, X_range, U_range, pasx, seed):
    ...
    pre_path = '/mimic_data/data_source/'
    Data = MIMICDataModule(gp_params, big_n_rct, big_n_obs, pre_path, seed + 4)
...
def get_estimates(dataset_train, dataset_val, delta, significance_level = 0.05):
    ...
    '''Param settinig'''
    X_columns = ['gender',	'age', 'heart rate', 'sodium', 'blood cells', 'glucose', 'hematocrit', 'respiratory rate']
    alpha = significance_level
    z_alpha = norm.ppf(1 - alpha/2)
    Y_train = np.stack(np.array(dataset_train['y']))
    A_train = np.array(dataset_train['A']).reshape(-1, 1)
    X_train = np.array(dataset_train[X_columns])
    n = Y_train.shape[0]
    d = X_train.shape[1]
    ...
    N_train = int(N/2)
    N_eval = N - N_train
    Y_N_train = np.array(dataset_val['y'])[:N_train]
    A_N_train = np.array(dataset_val['A']).reshape(-1, 1)[:N_train, :]
    X_N_train = np.array(dataset_val[X_columns])[:N_train, :]
    X_N_eval = np.array(dataset_val[X_columns])[N_train:, :]
    A_N_eval = np.array(dataset_val['A']).reshape(-1, 1)[N_train:, :]
    Y_N_eval = np.stack(np.array(dataset_val['y']))[N_train:].reshape(-1, 1)
    ...
    model_y = LinearRegression().fit(np.concatenate((X_N_train, A_N_train), axis=1), y_N_train)
    
    test_0 = np.concatenate((X_N_eval, np.zeros_like(A_N_eval)), axis=1)
    test_1 = np.concatenate((X_N_eval, np.ones_like(A_N_eval)), axis=1)
    mu0_N = model_y.predict(test_0).reshape(-1, 1)
    mu1_N = model_y.predict(test_1).reshape(-1, 1)

    aipw_N = (A_N_eval * Y_N_eval / e_N - (1 - A_N_eval) * Y_N_eval / (1 - e_N)) - \
    ...
    test_1 = np.concatenate((X_train, np.ones_like(A_train)), axis=1)
    mu0_n = model_y.predict(test_0).reshape(-1, 1)
    mu1_n = model_y.predict(test_1).reshape(-1, 1)

    aipw_n = (A_train * Y_train / e_n - (1 - A_train) * Y_train / (1 - e_n)) - \
            ((A_train - e_n) / e_n * (1 - e_n)) * ((1-e_n) * mu1_n + e_n * mu0_n)

    mean_rectifier = np.mean(aipw - aipw_n)
    var_rectifier = np.var(aipw - aipw_n)
    ...
    ate_est_obs = est_2.ate(X_N_eval)
