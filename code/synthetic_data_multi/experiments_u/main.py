...
def run_experiment(config_run, seed_list, methods_list, alpha):
    ...
    all_covs = ["X" + str(i+1) for i in range(500)] + ["U"]
    ...
    for ns, (om_A1_par, om_A0_par, w_sel_par, w_trt_par) in \
        ...
        for seed_index in range(len(seed_list)):
            ...
            ate_est, ate_ci = sim_cases(seed_list[seed_index], df_rct, df_obs, alpha, delta, all_covs)
            ...
            df.to_csv(f"{save_dir}/exp_results/alpha_{alpha}/unconfounding_{ns}/estimates_500_{seed_list[seed_index]}.csv")
   

if __name__ == "__main__":
    seed_list = [list(range(10)), list(range(20,30))]
    alpha_list = [0.05, 0.1]
    methods_list = ["normal_aipw", "normal_trial", "normal_ppi", "normal_obs"]
    config_run = load_yaml("/synthetic_data_multi/experiments_u/config_500")
