...
from synthetic_data_multi.synthetic_data_generation import *
...
def data_generation(gp_params, all_covs, big_n_rct, big_n_obs, n_MC, X_range, U_range, pasx, seed):
    ...
    X_mean_range = np.mean(X_range, axis=1)
    # gp_funcs["om_A0"] = sample_outcome_model_gp(X_mean_range, U_range, gp_params["om_A0_par"], seed + 0)  # GP - outcome model under treatment A=0
    # gp_funcs["om_A1"] = sample_outcome_model_gp(X_mean_range, U_range, gp_params["om_A1_par"], seed + 1)  # GP - outcome model under treatment A=1
    gp_funcs["om_A0"] = 0
    gp_funcs["om_A1"] = 1
    gp_funcs["w_sel"] = sample_outcome_model_gp(X_mean_range, U_range, gp_params["w_sel_par"], seed + 2)  # GP - selection score model P(S=1|X)
    gp_funcs["w_trt"] = sample_outcome_model_gp(X_mean_range, U_range, gp_params["w_trt_par"], seed + 3)  # GP - propensity score in OBS study P(A=1|X, S=2)

    SyntheticData = SyntheticDataModule(big_n_rct, big_n_obs, n_MC, gp_funcs, all_covs, X_mean_range, U_range, pasx, seed + 4)
...
def get_estimates(dataset_train, dataset_val, delta, all_covs, significance_level = 0.05):
    ...
    X_train = np.array(dataset_train[all_covs[:-1]])
    n = Y_train.shape[0]
    d = X_train.shape[1]

    '''IPW Implementation test'''
    e = estimate_e(X_train, A_train, LogisticRegression())
    mu0, mu1 = estimate_mu(X_train, A_train, Y_train, LinearRegression())
    ...
    N_train = int(N/2)
    N_eval = N - N_train
    Y_N_train = np.stack(np.array(dataset_val['y'])).reshape(-1, 1)[:N_train, :]
    A_N_train = np.array(dataset_val['A']).reshape(-1, 1)[:N_train, :]
    X_N_train = np.array(dataset_val[all_covs[:-1]])[:N_train, :]
    X_N_eval = np.array(dataset_val[all_covs[:-1]])[N_train:, :]
...
def sim_cases(seed,   df_rct, df_obs, significance_level, delta, all_covs):

        ate_estimates, ate_ci = get_estimates(df_rct, df_obs, delta, all_covs, significance_level)
